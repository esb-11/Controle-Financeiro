...
function expenseFactory(name, value, type, inputDate) {
  ...
  const date = inputDate ? new Date(inputDate.concat("T00:00")) : new Date();

  const id = crypto.randomUUID();
  
  return { id, name, value: valueParsed, type, paid, date };
}
